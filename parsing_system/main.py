...
def analyser(url: str):
    ...
    """
    initial_page = make_request(url=url)

    url_hash = get_url_hash(url=url)
    info_filename = f"./data/{url_hash}/configuration.json"
...
def scraper(url: str):
    ...
    analyser_data = get_json_parameters(url=url)

    url_hash = get_url_hash(url=url)
    directory_name = f"./data/{url_hash}/cache"
